...
const string strMessageMagic = "xdeus Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in xdeus
    bool fStrictPayToScriptHash = true; // Always active in xdeus
...
bool CBlock::AcceptBlock()
{
	...
	/*	xdeus is not using a checkpoint server
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock, std::string& strErr)
{
	...
	/* xdeus is not using checkpoint server
	...
	// presstab xdeus: enable of disable staking based on block difficulty
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "xdeus", CClientUIInterface::OK | CClientUIInterface::ICON_ETRKLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
		...
		/* xdeus does not use checkpoint server
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake && !pwalletMain->fDisableStake)  // attempt to find a coinstake && make sure settings allow PoS (presstab xdeus)
    {
