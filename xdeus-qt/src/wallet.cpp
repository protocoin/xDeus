    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s xd %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
						...
						// change xdeus to get rid of automatic generation of change address and instead send change back
...
{
    //presstab xdeus: return false if disablestake RPC is enabled
	if(fDisableStake)
		return false;
    ...
    // presstab xdeus - Initialize as static and don't update the set on every run of CreateCoinStake() in order to lighten resource use
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
    {
		...
		//xdeus now iterates each utxo inside of CheckStakeKernelHash()
		if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNewTime, nHashDrift, false, hashProofOfStake))
		{
			...
			//presstab xdeus - calculate the total size of our new output including the stake reward so that we can use it to decide whether to split the stake outputs
			uint64 nCoinAge;
			CTxDB txdb("r");
			const CBlockIndex* pIndex0 = GetLastBlockIndex(pindexBest, false);
			...
			//presstab xdeus - if MultiSend is set to send in coinstake we will add our outputs here (values asigned further down)
	...
	if(fCombineDust) //presstab xdeus - this combination code iterates through all of your outputs on successful coinstake, so its useful to have user be able to choose whether this is necessary
	{
		BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
		{
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64& nBalanceInQuestion, int& nOrphansFound, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %sxd %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %sxd %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), hash.ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
